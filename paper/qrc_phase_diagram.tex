...
The \QRCprojectlink{} provides the simulator, fixed grids, seeds, checked-in results, and figure scripts needed to reproduce every result.

\subsection{Implementation details}
We use exact density-matrix simulation without finite-shot sampling. Model selection uses validation NMSE only, and holdout is evaluated after band freezing. Confidence intervals use a percentile bootstrap with 50,000 resamples (seed 20260523). The strict band $B_{20,0.7}$ contains 22 grid coordinates, including nine on the $\gamma=0.12$ slice shown in Fig.~\ref{fig:evidence}.
